...
class SpectConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, edge_weight: OptTensor = None,
        ...
        if not self.depthwise:
            ...
            out = out + (1 + self.DSweight[0:1, :]) * self.propagate(edge_index, x=Tx_0, norm=edge_attr[:, 0],
                                                                     size=None)
            for i in range(1, enditr):
                out = out + self.DSweight[i:i + 1, :] * self.propagate(edge_index, x=Tx_0, norm=edge_attr[:, i],
                                                                       size=None)
...
class ML3Layer(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        if self.learnedge:
            tmp = torch.cat(
                [F.relu(self.fc1_1(edge_attr)), torch.tanh(self.fc1_2(edge_attr)) * torch.tanh(self.fc1_3(edge_attr))],
                1)
            edge_attr = F.relu(self.fc1_4(tmp))
        if self.nout2 > 0:
            x = torch.cat(
                [F.relu(self.conv1(x, edge_index, edge_attr)), torch.tanh(self.fc11(x)) * torch.tanh(self.fc12(x))], 1)
        else:
