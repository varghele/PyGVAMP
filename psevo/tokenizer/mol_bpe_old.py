...
from psevo.utils.chem_utils import molecule2smiles, smiles2molecule, get_submol
from psevo.utils.chem_utils import cnt_atom

from psevo.utils.token_utils import GeneralVocab, MAX_VALENCE
from psevo.utils.logger import print_log


class MolInPiece:
    """Manages molecular piece representation for BPE-like tokenization."""

    def __init__(self, mol):
        self.mol = mol
        self.smi = molecule2smiles(mol)

        # Initialize pieces: each atom starts as its own piece
        ...
        # Create inverse mapping: atom_id -> piece_id
        self.inversed_index = {}
        ...
        self.dirty = True
        self.smi2pids = {}  # Cache for neighboring pieces
    ...
    def get_nei_pieces(self):
        ...
        for key in self.pieces:
            ...
            # Find neighboring pieces through atom connections
            for aid in piece:
    ...
    def get_nei_smis(self):
        """Get SMILES representations of all possible neighboring piece merges."""
        if self.dirty:
    ...
    def merge(self, smi):
        """Merge pieces that form the specified SMILES pattern."""
        if self.dirty:
            self.get_nei_smis()
    ...
    def get_smis_pieces(self):
        """Get final pieces as (SMILES, atom_indices) tuples."""
        res = []
...
def graph_bpe(fname, vocab_len, vocab_path, cpus):
    """Molecular piece extraction using BPE-like algorithm."""
    # Load molecules
    print_log(f'Loading mols from {fname} ...')
    with open(fname, 'r') as fin:
        smis = list(map(lambda x: x.strip(), fin.readlines()))

    # Initialize molecules with piece management (filter out molecules with '+')
    ...
    # Calculate atom frequencies by simple character counting
    for smi in smis:
        for c in smi:
            if c in details:
                details[c][1] += 1
...
class Tokenizer:
    """Molecular piece tokenizer for BPE-based representation."""
    ...
    def tokenize(self, mol, return_idx=False):
        ...
        # Construct adjacency matrix between pieces (for potential ordering)
        aid2pid = {}
        ...
        # Random shuffling (could use BFS ordering instead)
...
def parse():
    """Parse command line arguments for molecular BPE."""
